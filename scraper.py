...
def main():
    # 🔗 広告業界・アドテクの主要ニュース配信ルート（超強化版）
    rss_urls = [
        "https://rss.rssad.jp/rss/markezine/new/markezine.xml",     # 🔥 MarkeZine（マーケティング・広告専門の最大手）
        "https://webtan.impress.co.jp/rss/all.xml",                # 🔥 Web担当者Forum（デジタルマーケ・広告動向）
    ...
    if not target_articles:
        print("❌ 重要キーワードにヒットする記事がありませんでした。")
        return
        
    success_count = 0
    # 3. 専門メディア情報も含めて一気に5件同時開通！
    for article in target_articles:
        ...
        try:
            ...
            if "サイバーエージェント" in article['title'] or "aja" in article['title']:
                ...
                category = "競合企業情報"
                product = "ミエルTV"
            elif "トレンド" in article['title'] or "ai" in article['title']:
                category = "その他トレンド"

            # 営業・企業研究に直結する構造化テキスト
            imp = "A"
            summary = f"{article['title']}に関する、デジタルおよびテレビ広告領域における重要な最新動向です。"
            opp = "大手企業の動きや市場変化に対し、AJA独自のCTV配信技術（incrie）や地上波効果可視化（ミエルTV）を組み合わせた柔軟なプランニングで差別化し、新規獲得のチャンスです。"
